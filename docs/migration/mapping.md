...
### `beaconHS.SETTINGSLOCATION` → `org_units(level='site')` _(EAV pivot — `transform`)_
...
_The mapper must first `SELECT DISTINCT MetaKey` to enumerate the key set._ Parent `customer`/`project`
...
### `beaconHS.HAZIDJSA` → `hazid_assessments` (children: tasks/hazards/ppe/questions/signatures/photos/cs\_\*)
...
### `toolCRIB.EQUIPMENT` → `equipment_items` (+ `equipment_types`, `equipment_categories`) _(`transform`)_
...
_The remaining `map` tables (documents, ppe, inspections children, equipment logs, people lookups,
...
`gap`/`review` items are detailed in [`gaps.md`](./gaps.md)._
