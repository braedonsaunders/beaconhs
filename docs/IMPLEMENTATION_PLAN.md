...
- ✅ **People depth** — `/people/groups`, `/people/divisions` (hierarchical), `/people/titles` (with Job Description tab + task list + per-task acknowledgement matrix), per-title PDF. Detail page adds Groups / Divisions / Title / Compliance tabs (compliance shows training% + doc-ack% + PPE roster + overdue count).
