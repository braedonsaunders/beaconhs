      ...
      {mounted && open ? (
        <div key="drawer" className="fixed inset-0 z-50">
          ...
          <motion.aside
            ...
            className={cn(
              'absolute right-0 top-0 flex h-full flex-col overflow-hidden border-l border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900',
              SIZE_CLASS[size],
            )}
          >
            {(title || description) ? (
            ...
            <div className="app-scroll min-h-0 flex-1 overflow-y-auto px-6 py-5 text-slate-900 dark:text-slate-100">{children}</div>
            {footer ? (
