...
//
// If a generated work order PDF already exists, redirect to a short-lived
...
import { requestEquipmentWorkOrderPdf } from '@/lib/pdf-actions'
...
export async function GET(
    ...
    const [row] = await tx
      ...
      .where(
        and(
          ...
          like(attachments.r2Key, `pdfs/equipment-work-orders/${id}-%`),
        ),
      )
  ...
  const result = await requestEquipmentWorkOrderPdf(id)
  if (!result.ok) {
