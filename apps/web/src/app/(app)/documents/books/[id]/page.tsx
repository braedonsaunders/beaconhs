          ...
          <>
            <Card>
              ...
              <CardContent>
                {items.length === 0 ? (
                  ...
                  <ReorderableList
                    key={items
                      .map((row) => `${row.doc.id}:${row.item.documentVersionId ?? 'draft'}`)
                      .sort()
                      .join('|')}
                    bookId={id}
            ...
            {book.status === 'draft' ? (
                ...
                <form action={addDocumentToBook} className="flex items-end gap-2">
                  ...
                  <div className="min-w-0 flex-1 space-y-1.5">
                    ...
                    <RemoteSelectField
                      ...
                      placeholder="Pick a published document…"
              ...
              <p className="text-sm text-slate-600 dark:text-slate-300">
                This published book is locked to exact document versions. Unpublish it before
                changing the contents or order.
              </p>
            )}
          </>
        ) : null}
              ...
              <form action={updateBookSettings} className="space-y-4 text-sm">
                ...
                {book.status === 'draft' ? (
                  ...
                  <p className="border-t border-slate-100 pt-4 text-slate-600 dark:border-slate-800 dark:text-slate-300">
                    Published book settings are locked. Click <strong>Unpublish</strong> before
                    editing them.
                  </p>
                )}
              </form>
