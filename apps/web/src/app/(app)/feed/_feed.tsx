  ...
  const rendered = useMemo(() => {
    ...
    for (const e of events) {
      if (mounted) {
        ...
        if (b !== lastBucket) {
          out.push(<DaySeparator key={`sep-${b}-${e.id}`} label={tGeneratedValue(b)} />)
          lastBucket = b
        }
      }
      out.push(
    ...
    return out
  }, [events, locale, mounted, tGeneratedValue])
        ...
        value={
          summary.today > 0 ? (
            <p className="mt-1 inline-flex items-center gap-1.5 text-xs font-medium text-emerald-600 dark:text-emerald-400">
              ...
              <GeneratedValue value={summary.today} /> <GeneratedText id="m_01368eafe86ff1" />
            </p>
          ) : null
        }
      />
...
function LegendCard() {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <h2 className="text-sm font-semibold text-slate-900 dark:text-slate-100">
        <GeneratedText id="m_035f34bd804acd" />
      </h2>
      <p className="mt-1 text-xs leading-relaxed text-slate-500 dark:text-slate-400">
        <GeneratedText id="m_188df43dabd8a1" />
      </p>
          ...
          value={KIND_ORDER.map((k) => {
            ...
            return (
              ...
              >
                ...
                <GeneratedValue value={meta.plural} />
              </li>
            )
          })}
        />
    ...
    <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md motion-reduce:transition-none motion-reduce:hover:translate-y-0 dark:border-slate-800 dark:bg-slate-900 dark:hover:border-slate-700">
          ...
          <div className="min-w-0 flex-1 pt-0.5">
            <p className="text-[15px] leading-snug text-slate-700 dark:text-slate-300">
              <span className="font-semibold text-slate-900 dark:text-slate-100">
                <GeneratedValue value={actor} />
              </span>
              <GeneratedValue value={' '} />
              <span className="text-slate-500 dark:text-slate-400">
                <GeneratedValue value={event.action} />
              </span>
            </p>
            <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-slate-400 dark:text-slate-500">
              ...
              >
                <k.Icon size={11} /> <GeneratedValue value={k.label} />
              </span>
              <time suppressHydrationWarning dateTime={event.at}>
                <GeneratedValue value={timeAgo(event.at, locale)} />
              </time>
              <GeneratedValue
                value={
                  event.siteName ? (
                    <span className="inline-flex items-center gap-1">
                      ...
                      <MapPin size={11} /> <GeneratedValue value={event.siteName} />
                    </span>
                  ) : null
                }
              />
            ...
            value={
              event.badge ? (
                <span className="shrink-0 rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-600 ring-1 ring-slate-200/70 ring-inset dark:bg-slate-800 dark:text-slate-300 dark:ring-slate-700">
                  <GeneratedValue value={event.badge} />
                </span>
              ) : null
            }
          />
      ...
      >
        <span className="inline-flex items-center gap-1.5">
          <k.Icon size={13} /> <GeneratedText id="m_107ab58c3c38bc" />{' '}
          <GeneratedValue value={k.label.toLowerCase()} />
        </span>
...
function PhotoGrid({
  ...
  if (n === 1)
    return (
      <div className={ring}>
        <GeneratedValue value={cell(shown[0]!, 0, 'h-72 w-full sm:h-80')} />
      </div>
    )
  if (n === 2)
    return (
      <div className={cn(ring, 'grid grid-cols-2 gap-1')}>
        <GeneratedValue value={shown.map((u, i) => cell(u, i, 'h-56'))} />
      </div>
    )
  if (n === 3)
    return (
      <div className={cn(ring, 'grid grid-cols-2 grid-rows-2 gap-1')}>
        <GeneratedValue value={cell(shown[0]!, 0, 'col-span-2 h-52')} />
        <GeneratedValue value={cell(shown[1]!, 1, 'h-40')} />
        <GeneratedValue value={cell(shown[2]!, 2, 'h-40')} />
      </div>
    )
  return (
    <div className={cn(ring, 'grid grid-cols-2 gap-1')}>
      <GeneratedValue value={shown.map((u, i) => cell(u, i, 'h-40'))} />
    </div>
  )
}
        ...
        value={
          many ? (
            <div className="absolute bottom-5 left-1/2 -translate-x-1/2 rounded-full bg-white/10 px-3 py-1 text-sm font-medium text-white/90 tabular-nums">
              <GeneratedValue value={index + 1} /> / <GeneratedValue value={urls.length} />
            </div>
          ) : null
        }
      />
...
function LoadingMore() {
  return (
    <div className="flex items-center justify-center gap-2 py-8 text-sm text-slate-400 dark:text-slate-500">
      <Loader2 size={15} className="animate-spin" /> <GeneratedText id="m_06a34cb964735d" />
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col items-center gap-1.5 py-10 text-center">
      ...
      <p className="text-xs font-medium text-slate-400 dark:text-slate-500">
        <GeneratedText id="m_18060e8330c3ef" />
      </p>
    </div>
  )
}
...
function EmptyFeed({ filter, onClear }: { filter: Filter; onClear: () => void }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-white/50 px-6 py-20 text-center dark:border-slate-800 dark:bg-slate-900/40">
      ...
      <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">
        <GeneratedText id="m_117d1a5e1ef440" /> <GeneratedValue value={meta.plural.toLowerCase()} />{' '}
        <GeneratedText id="m_06f587cd3e4acd" />
      </h2>
      <p className="mt-1.5 max-w-sm text-sm leading-relaxed text-slate-500 dark:text-slate-400">
        <GeneratedText id="m_0a88fe6a417896" />
      </p>
      <button
        type="button"
        onClick={onClear}
      ...
      >
        <Layers size={14} /> <GeneratedText id="m_18146d74b511c4" />
      </button>
    </div>
  )
}
