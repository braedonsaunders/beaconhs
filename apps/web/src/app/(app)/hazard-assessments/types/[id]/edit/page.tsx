...
import { hazidAssessmentTypes, hazidHazardSets, personGroups } from '@beaconhs/db/schema'
...
async function update(formData: FormData) {
  ...
  redirect(`/hazard-assessments/types/${id}`)
}
...
export default async function EditAssessmentTypePage({
  ...
  const data = await ctx.db(async (tx) => {
    ...
    return { row, sets, groups }
  })
  if (!data) notFound()
  const { row, sets, groups } = data
  return (
    <PageContainer>
      <div className="max-w-2xl space-y-6">
        <DetailHeader
          back={{ href: `/hazard-assessments/types/${id}`, label: 'Back' }}
              ...
              {groups.length > 0 ? (
                <div className="space-y-1.5">
                  ...
                  <MultiPicker
                    ...
                    defaultSelected={row.availableToGroupIds ?? []}
