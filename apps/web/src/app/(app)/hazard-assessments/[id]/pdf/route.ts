...
//
// If a generated HazID assessment PDF already exists, redirect to a short-lived
...
import { requestHazidPdf } from '@/lib/pdf-actions'
...
export async function GET(
  ...
  // The wave-6 worker writes attachments with filenames matching `hazid-…`
  // and r2 keys under `pdfs/hazard-assessments/<assessmentId>-…`. Look up the latest by
  // r2 key prefix so multi-renders return the freshest copy.
    ...
    const [row] = await tx
      ...
      .where(
        and(
          ...
          like(attachments.r2Key, `pdfs/hazard-assessments/${id}-%`),
        ),
      )
  ...
  const result = await requestHazidPdf(id)
  if (!result.ok) {
