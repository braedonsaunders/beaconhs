// GET /lift-plans/:id/pdf
//
// If a generated lift plan PDF already exists, redirect to a short-lived
...
import { requestLiftPlanPdf } from '@/lib/pdf-actions'
...
export async function GET(
    ...
    const [row] = await tx
      ...
      .where(
        and(
          ...
          like(attachments.r2Key, `pdfs/lift-plans/${id}-%`),
        ),
      )
  ...
  const result = await requestLiftPlanPdf(id)
  if (!result.ok) {
