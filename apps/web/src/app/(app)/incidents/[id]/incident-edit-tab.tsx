...
// Inline edit surface for an incident — the detail page's "Edit" tab. One page
// = the detail view + edit surface; /incidents/[id]/edit only redirects here.
// Status changes, photos, injuries, investigation and CAs live in other tabs.
// Stays on the page after saving (revalidate, no redirect). Honours `locked`.
...
async function saveIncident(formData: FormData): Promise<void> {
  'use server'
...
export async function IncidentEditTab({ incidentId }: { incidentId: string }) {
  const ctx = await requireRequestContext()
    ...
    <div className="space-y-5">
      {inc.locked ? (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/40 dark:text-amber-200">
          This incident is <strong>locked</strong>. Unlock it from the overview tab before editing.
        </div>
      ) : null}
      <form
        action={saveIncident}
        className="space-y-5 rounded-lg border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900"
      >
