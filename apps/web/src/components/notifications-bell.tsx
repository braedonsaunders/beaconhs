    ...
    <Popover
      ...
      trigger={
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          aria-label={tGeneratedValue(t('notifications'))}
          aria-expanded={open}
          aria-haspopup="menu"
            ...
            value={
              unread > 0 ? (
                <span className="absolute top-1 right-1 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-red-600 px-1 text-[10px] leading-none font-medium text-white">
                  <GeneratedValue value={unread > 99 ? '99+' : unread} />
                </span>
              ) : null
            }
          />
      ...
      <div role="menu">
        <div className="flex items-center justify-between border-b border-slate-100 px-3 py-2 dark:border-slate-800">
          <div className="text-sm font-medium">
            <GeneratedValue value={t('notifications')} />
          </div>
          <GeneratedValue
            value={
              unread > 0 ? (
                <span className="text-xs text-slate-500 dark:text-slate-400">
                  <GeneratedValue value={t('unread', { count: unread > 99 ? 99 : unread })} />
                </span>
              ) : (
                <span className="text-xs text-slate-400 dark:text-slate-500">
                  <GeneratedValue value={t('allRead')} />
                </span>
              )
            }
          />
        ...
        >
          ...
          <span>
            <GeneratedValue value={t('viewInbox')} />
          </span>
        </Link>
        <Link
        ...
        >
          ...
          <span>
            <GeneratedValue value={t('preferences')} />
          </span>
        </Link>
      </div>
    </Popover>
