...
describe('in-app issue reporter contract', () => {
  ...
  it('keeps the turn route tenant-scoped and destination-gated', () => {
    ...
    expect(route).toContain('createScriptedFeedbackClient')
    expect(route).toContain('feedbackGithubRequest')
    expect(route).toContain('feedbackFiledAuditEvent')
    expect(route).toContain('failed to persist turn')
    expect(route).toContain('filing without the model')
    expect(route).not.toContain('entityId: result.issue.id')
