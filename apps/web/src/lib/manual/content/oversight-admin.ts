...
It also shows a **Booked for** date when that person already has a seat on an upcoming class for that course. Coverage still reads **missing** or **expired** — nobody is covered until they sit the course — but a dated **Booked for** means it is already handled and does not need chasing. Rows with no date are the ones to act on. Filter on **Booked** to show only one group or the other. A cancelled or completed class does not count as booked.
