...
export type ToolResult =
  | { ok: true; data: unknown; note?: string }
  | { ok: false; error: string }

export type AssistantToolDef<TArgs = unknown> = {
  ...
  /** Per-call handler. Receives the request context; opens its own RLS tx via
   *  ctx.db(). Must return a ToolResult and must not throw. */
  execute: (args: TArgs, ctx: RequestContext) => Promise<ToolResult>
}
