...
describe('scheduled report cadence', () => {
  ...
  it('anchors multi-week schedules to the configured start date', () => {
    expect(
      computeNextRunAt(
        {
          cadence: 'weekly',
  ...
  it('supports nth-weekday monthly schedules', () => {
    expect(
      computeNextRunAt(
        {
          cadence: 'monthly',
    ...
    ).toBe('2026-05-04T10:30:00.000Z')
  })

  it('returns no occurrence after an end date', () => {
    expect(
      computeNextRunAt(
        {
          cadence: 'weekly',
